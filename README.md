...
> **결과 측정 방식** — AI Perception 4종 × 큐브 배치 2종, 총 8개 경우의 수에 대한 각각 10회 성공/실패 여부


AI기반 Perception 방식은 YOLO+SDG 외에 zero-shot 계열 3가지까지 총 4가지 방식으로 구현했고, 각각의 성능을 비교하고자 하였으며

기본적으로 인식을 수행해서 빨, 노, 연, 파 순서대로 쌓는 것에 대해
과제에 명세된 환경으로는, 이 4가지 방식의 성능을 비교하기 어려워


문제의 상황을 조금 더 실제 환경처럼 구성 (Advanced Ver.)해서도 함께 비교해서 풀었습니다.
...
4편 모두 같은 랜덤 배치(같은 seed)에서의 완주 회차라
소스 간 직접 성공/실패 비교 가능 (ZED 카메라 시점 + 검출 오버레이):
...
- **[③ 판단·제어](docs/03_decision_control.md)** — Perception 결과를 안전장치(신뢰도 게이트 · EMA 필터 · 조작 중 동결 · 작업공간 게이트 · 탑 보호)로 걸러 로봇이 인식하는 Belief 쪽 데이터를 갱신하고, 다음 행동을 결정해 로봇을 움직임
...
Cortex 가 인식 연동용으로 제공하는 `CortexObject.set_measured_pose()`
API 안에서 통합 과정을 진행함.

의사결정 로직은 쌓기 순서 설정 외에는 수정하지 않음.
...
같은 검증 이미지 300장 · 같은 채점 코드로 실측 비교함. 


네 방식 모두 GUI 드롭다운으로 **실행 중 전환**됨.
...
- Perception 성능지표(mAP)와 E2E 성공률이 꼭 일치하지만은 않는다 (위 결과 표). 큐브가 기본 배치일 때는 mAP 순서와
  비슷하지만 (GDINO/Qwen 10/10, YOLO-World 6/10), 랜덤하게 큐브가 배치된 (Advanced Ver.) 조건에서는 mAP 0.878 인
  GDINO 가 4/10 로 떨어지고 가장 느린 Qwen(~0.2Hz)이 10/10. 정확도/지연/신뢰도
  보정/안전장치가 함께 작용하기 때문에 E2E 실측 없이는 판단이 어렵다.
