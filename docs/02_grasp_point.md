...
**입력** [①](01_perception.md)의 박스 + 깊이맵 → **출력** 큐브 중심의 월드 좌표
(x, y, z) + 자세(yaw).
...
1. **깊이 샘플링** — 박스 중앙 40% 영역의 깊이 **중앙값** (가장자리·배경 픽셀 배제)
2. **역투영** — 박스 중심 픽셀 + 깊이로 카메라 광선을 따라 3D 점 복원 = 큐브 앞면
