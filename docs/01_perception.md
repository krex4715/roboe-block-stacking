...
**입력** ZED-X RGB 프레임 → **출력** 색상별 큐브의 클래스 + 바운딩박스 + 신뢰도.
매 6 물리스텝(10Hz)마다 실행 — 인식이 렌더링과 GPU 를 나눠 쓰므로(단독 2.2ms →
공유 27ms) 더 자주 돌릴 이유가 없음.
...
런타임과 **같은 씬을 같은 카메라**로 찍어 생성 — 학습-실전 차이(도메인 갭)가
원리적으로 없음.

- 무작위화: 큐브 위치·회전, 로봇 자세, 조명, 카메라 미세 지터
- train 2,800장 / val 300장 (별도 시드)
- 결과: **mAP50 0.9949**, 노랑↔연두 오분류 600건 중 1건

## 배포 — Isaac Sim 환경에 추가 패키지 0개

학습은 별도 venv(ultralytics), 런타임은 **TorchScript** 로 내보내 순정 torch 만으로 로드.
