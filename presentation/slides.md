...
- 제시로 끝내지 않고 **GUI "인식 소스" 드롭다운으로 4종 라이브 전환** (Qwen VLM 포함) — **G-DINO 만으로 완주 실증** (비동기 2.7Hz, ghost belief 가 저속 인식을 견딤)

<!--
"VLM 같은 걸로 zero-shot 대안 제시" 요청에 대한 직답 장표.
- 라이브 통합: YOLO-World 는 CLIP 임베딩을 bake 한 TorchScript(기존 배포 경로 재사용, parity 1.00000),
  G-DINO 는 학습 venv 서브프로세스 워커(JSON 라인) - "isaacsim 신규 패키지 0" 원칙 유지.
- 게이트 자동 전환 0.5/0.003/0.25: 신뢰도는 보정 특성이라 절대값 비교 불가. YOLO-World TP 중앙값
  0.0105(!), TP/오검 분포 겹침 - 게이트로 분리 불가 -> 보호는 bridge 안전장치가 맡는다.
- E2E: G-DINO 단독 완주 25.5s(기본 스폰 1회 실증 - 랜덤 배치평가 10/10 은 파인튜닝 쪽. 범위 정직하게).
