...
## 8. 확장 — zero-shot 으로 되나? 4개 인식 소스 실측 비교

| 인식 소스 | 방식 | mAP50 | pick 정확도* | 지연/장 |
|---|---|---|---|---|
| **YOLOv8n 파인튜닝** ✅ 기본 | 폐쇄셋 학습 (SDG) | **0.999** | **0.999** | **7 ms** |
...
<span class="small">같은 val 300장 · 같은 채점기 (채점 구현 차이 원천 배제) · (*) pick = 브리지 정책(클래스별 최고 신뢰도 1개)이 진짜 그 색 큐브에 맞은 비율 · Qwen 은 30장 서브셋</span>

- 연두→노랑 혼동 **67회**(G-DINO) · 프롬프트 민감도 0.53→0.68(YOLO-World) — 기각 사유가 **수치**가 됨
- zero-shot 신뢰도는 **무보정** — conf 0.25 게이트에서 전멸, 고정 게이트에 그대로 못 꽂는다
- 결론: **대체가 아니라 역할 분담** — runtime 은 파인튜닝 소형 모델, zero-shot 은 실환경 **auto-labeling(교사) → 증류(학생)**

<!--
"VLM 같은 걸로 zero-shot 대안 제시" 요청에 대한 직답 장표. 막대그림은 media/figures/zeroshot_compare.png.
- Qwen2.5-VL(진짜 VLM)은 색 판단 최강(혼동 3/120)이지만 3.9s/장 — 루프가 아니라 오프라인 분석/라벨링용.
- 프롬프트 원문은 README §3 수록 (YOLO-World 어휘 목록 / G-DINO 마침표 캡션 / Qwen JSON 지시문).
...
- 용어 주의: VLA 아님 — VLA 는 행동을 직접 출력하는 정책 모델. 우리는 박스를 출력하는 VLM/open-vocab 검출기.
-->

---

## 9. 확장 — 제시로 끝내지 않고, 4종 라이브 전환

**GUI "인식 소스" 드롭다운**: 파인튜닝 YOLO ↔ YOLO-World ↔ G-DINO ↔ Qwen2.5-VL
— "검출기는 RGB→클래스+박스 **생산자 자리 하나**"라는 구조 주장의 **실연**

- **게이트가 백엔드를 따라감**: 0.5 / 0.003 / 0.25 / 고정 0.99 자동 전환
  — 신뢰도는 모델별 보정 특성이라 절대값 비교 불가
- 느린 모델은 **비동기 우편함** (시뮬 루프 무차단) · 전환 시 이전 워커 먼저 종료 (VRAM 순차 점유)
- **실증: G-DINO(zero-shot)만으로 스태킹 완주** — 시뮬 25.5s, 실효 2.4~2.9Hz
  → ghost belief 가 **저속 인식에도 강건**하다는 설계 주장의 직접 증거

<!--
- YOLO-World 는 CLIP 임베딩을 bake 한 TorchScript(기존 배포 경로 재사용, parity 1.00000),
  G-DINO/Qwen 은 학습 venv 서브프로세스 워커(JSON 라인) - "isaacsim 신규 패키지 0" 원칙 유지.
- 0.003 근거: YOLO-World TP argmax 신뢰도 중앙값 0.0105, TP/오검 분포 겹침 → 게이트로 분리 불가,
  보호는 bridge 안전장치(작업공간/일관성 스냅/동결)가 맡는다.
- Qwen 은 신뢰도 자체가 없어 고정 0.99 발행 + bridge 전담. ~0.2Hz, VRAM ~7GB 상주.
- G-DINO 완주는 기본 스폰 1회 실증 - 랜덤 배치평가 10/10 은 파인튜닝 쪽 (범위 정직하게).
- YOLO-World 라이브는 프레임당 0~3개/점수 ~0.01로 흔들림 = 오프라인 pick 0.70 의 실물.
-->
...
## 12. 통합 원칙 — "조작 중에는 belief 를 건드리지 않는다"
...
**두 문제의 성공률** — 본연(기본 일자 배치): **5/5 연속 완주** · 스스로 틀어버린 문제(랜덤+오염): **10/10**

"**한 번 되는 것**과 **항상 되는 것**은 다른 문제" — 강건성 결함은 전부 스트레스 조건이 끌어냈고,
명세 조건만 검증했다면 **데모 중에 처음** 만났을 결함들

<!--
본연 시나리오(스톡 배치) 5연속은 M5 게이트에서 별도 검증. 요구 축소가 아니라는 방어선.
-->

---

## 14. 강건성 여정 — 배치 평가 22% → 100%
...
| 지표 | 값 |
|---|---|
| **기본 배치** (과제 명세 그대로) | **5/5 연속 완주**, 회당 ~35s, 탑 오차 1~8mm |
| **랜덤 배치** (위치+yaw 무작위, belief 오염 시작) | **10/10 (100%)**, 평균 44.9s, 탑 오차 ≤7.4mm |
| 인식 정확도 (정지 120측정) | 평균 **2.4mm** / p95 4.2mm / 미검출 0 |
| 추론 지연 | 단독 2.2ms / 렌더 동시 **27ms (12배)** → 10Hz 설계 근거 |

<!--
바닐라(기본 일자 배치) vs 틀어버린 문제(랜덤) — 두 성공률을 나란히 보여주는 것이 포인트.
